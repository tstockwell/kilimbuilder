...
/**
 * This class takes the basic blocks from a MethodFlow and generates all the
 * extra code to support continuations.
 */
...
public class MethodWeaver {
	 ...
	 * 
	 * @param bb
	 *            The BasicBlock that contains the pausable method invocation as
	...
	/*
	 * The Task.getCurrentTask() method is marked pausable to force the caller
	 * to be pausable too. But the method doesn't really pause; it merely looks
	 * up the task from the fiber. This is a special case where the call to
	 * getCurrentTask is replaced by <pre> load fiberVar getfield task
	 * 
	 * @param mv
	 */
	...
	private void genPrelude(MethodVisitor mv) {
		...
		if (callWeavers.size() == 0 && (!hasGetCurrentTask())) {
			return; // No pausable methods, no getCurrentTask. Prelude is not
					// needed at all.
		}
	...
	/*
	 * The number of words in the argument; doubles/longs occupy two local vars.
	 */
	...
	/**
	 * Generate code for only those catch blocks that are reachable from one or
	 * more pausable blocks. fiber.pc tells us which nested call possibly caused
	 * an exception, fiber.status tells us whether there is any state that needs
	 * to be restored, and fiber.curState gives us access to that state.
	 * 
	 * ; Figure out which pausable method could have caused this.
	 * 
	 * switch (fiber.upEx()) { 0: goto NORMAL_EXCEPTION_HANDLING; 2: goto
	 * RESTORE_F } RESTORE_F: if (fiber.curStatus == HAS_STATE) { restore
	 * variables from the state. don't restore stack goto
	 * NORMAL_EXCEPTION_HANDLING } ... other RESTOREs
	 * 
	...
	private void genException(MethodVisitor mv, BasicBlock bb, List<CallWeaver> cwList) {
		...
		for (CallWeaver cw : cwList) {
			if (i > 0) {
				// This is the jump (to normal exception handling) for the
				// previous
				// switch case.
	...
	void makeNotWovenMethod(ClassVisitor cv, MethodFlow mf) {
		...
		MethodVisitor mv = cv.visitMethod(access, mf.name, mf.desc, mf.signature, ClassWeaver.toStringArray(mf.exceptions));
		mv.visitCode();
		visitAttrs(mv);
		...
		if ((mf.access & Constants.ACC_ABSTRACT) != 0) {
			// The abstract method doesn't contain the number of locals required
			// to hold the
